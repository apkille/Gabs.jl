@testitem "Non-Gaussian States" begin
    ...
    @testset "Cat States - Basic Properties" begin
        ...
        @test isapprox(cat_even.coefficients, cat_even_check.coefficients, atol=1e-12)
        
        cat_odd_check = catstate(basis, α, π)
        @test isapprox(cat_odd.coefficients[1], cat_odd_check.coefficients[1], atol=1e-12)
        @test isapprox(cat_odd.coefficients[2], cat_odd_check.coefficients[2], atol=1e-12)
    end
    ...
    @testset "Cat States - Normalization" begin
        ...
        for α in [0.5, 1.0, 1.5, 2.0]
            ...
            norm_even = sqrt(sum(abs2, cat_even.coefficients))
            norm_odd = sqrt(sum(abs2, cat_odd.coefficients))
            
            if α > 1.0
                @test abs(cat_even.coefficients[1] - cat_even.coefficients[2]) < 1e-10
                @test abs(abs(cat_odd.coefficients[1]) - abs(cat_odd.coefficients[2])) < 1e-10
            end
        end
    end
    ...
    @testset "Cat States - Edge Cases" begin
        ...
        @test abs(cat_even_small.coefficients[1]) < abs(cat_odd_small.coefficients[1])
        ...
        @test abs(abs(cat_even_large.coefficients[1]) - expected_coeff) < 0.01
        @test abs(abs(cat_odd_large.coefficients[1]) - expected_coeff) < 0.01
    ...
    @testset "Integration with Existing Framework" begin
        ...
        original_norm = sqrt(sum(abs2, cat_copy.coefficients))
        Gabs.normalize!(cat_copy)
        new_norm = sqrt(sum(abs2, cat_copy.coefficients))
        ...
        @test all(abs.(cat_scaled.coefficients) .≈ 0.5 * abs.(cat1.coefficients))
        
        cat_zero = zero(cat1)
        @test cat_zero isa GaussianLinearCombination
